...
Matrix createLocalK(int e,mesh &m){
    ...
    float constT,kap,constD,constL,constN,Ae,J,D;
    ...
    productRealMatrix((constT*J)/(24*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,Beta,2,2,6),6,2,6),matrixA);

    //Preparando matrixK 
    kap = m.getParameter(KAP);
    Ae = calculateLocalArea(e,m);
    transpose(Alpha,Alphat);
    transpose(Beta,Betat);
    productRealMatrix((kap*Ae)/(D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,Beta,2,2,6),2,2,6),6,2,6),matrixK);

    //Preparando matrixG 
    constD = m.getParameter(CONSTD);
    calculateBPrima(BPrima);
    productRealMatrix(-(constD*Ae)/(8*(D*D)),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixG);


    //Preparando MatrixH
    constL = m.getParameter(CONSTL);
    productRealMatrix((constL*J)/(9*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixH);
...
Vector createLocalb(int e,mesh &m){
    Vector b0,b,f;
    Matrix g_matrix;
    float constN = m.getParameter(CONSTN);
    ...
    b.push_back((constN*J)/6); b.push_back((constN*J)/6); b.push_back((constN*J)/6);
